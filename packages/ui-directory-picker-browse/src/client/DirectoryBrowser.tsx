...
import {
  Button, IconCheckOutline16, IconChevronRightOutline14, IconChevronUpOutline14, IconCloseOutline16,
...
import { directoryParentPath, displayCrumbs } from './directory-path.ts'

/** Owner-supplied browser props: browse calls, pick semantics, and copy. */
export interface DirectoryBrowserProps {
  /** Stable directory target identity; changing it cancels the old scan without closing the dialog. */
  targetKey: string
  ...
  /** Localized copy. */
  t: Translate
  /** Optional caller-owned controls displayed above the path browser. */
  header?: ReactNode
}
...
function failureText(error: unknown): string {
  if (error !== null && typeof error === 'object') {
    ...
    if (rpcError !== null && typeof rpcError === 'object' && typeof (rpcError as { message?: unknown }).message === 'string') {
      return (rpcError as { message: string }).message
    }
  }
 ...
 */
export function DirectoryBrowser({ targetKey, open, listDirectory, createDirectory, onOpen, onClose, busy, t, header }: DirectoryBrowserProps) {
  ...
  // Every open or Host target starts fresh at that Host's home directory.
  // A target change aborts the old scan while keeping this Modal mounted, so
  // the select change cannot fire the owner's cancellation path.
  useEffect(() => {
    openGeneration.current += 1
    if (open) {
      setParent(null)
    ...
    refocusPick.current = false
    refocusEditZone.current = false
  }, [open, targetKey, navigate, supersede])
  ...
  const roots = crumbSource?.roots ?? []
  const upPath = crumbSource === null ? undefined : directoryParentPath(crumbSource)
  ...
  // The create overlay owns the interaction while open: Modal has no
  ...
  return (
    <>
      <Modal
        ...
        // the create overlay is up only that topmost dialog may close
      ...
      >
        <div className={css.header}>
          ...
          {header}
          <div className={css.sourceLabel}>{t('browser.sourceFolder')}</div>
          {roots.length > 1 && (
            <div className={css.rootBar} aria-label={t('browser.roots')}>
              {roots.map(root => (
                <button
                  key={root.path}
                  type="button"
                  className={clsx(css.rootButton, crumbs[0]?.path === root.path && css.rootButtonActive)}
                  disabled={parentInert}
          ...
          <div className={css.pathRow}>
            <button
              ...
              title={t('browser.up')}
              disabled={parentInert || upPath === undefined}
              onClick={() => { if (upPath !== undefined) navigate(upPath) }}
            >
              ...
              {pathDraft === null
                ? (
                  <>
                    ...
                    >
                      <span className={css.pathDisplay}>{targetPath ?? ''}</span>
      ...
      {/* The create dialog is a sibling overlay rather than a child of the
        * directory-picker Modal. Keeping the two portal owners independent
        * guarantees that closing a successful create only dismisses this
        * short-lived overlay; the New Project picker stays mounted with its
        * directory state and can refresh/select the folder that was created. */}
      <Modal
