...
export function createCodexSearchProvider(options) {
  ...
  return Object.freeze({
    ...
    async search(request, signal) {
      const resolved = options.resolveCredentials ? await options.resolveCredentials({ signal }) : undefined
      const auth = resolved?.auth ?? await options.getAuth({ signal })
      const credential = resolved?.credential ?? await options.readCredential({ signal })
