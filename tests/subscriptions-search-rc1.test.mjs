...
import { searchSession } from '../packages/subscriptions-compat-rc1/src/search.js'

import {
  CODEX_SEARCH_PROVIDER_ID,
  CODEX_SEARCH_URL,
  createCodexSearchProvider,
} from '../packages/subscriptions-compat-rc1/src/codex-search.js'

test('search uses one account snapshot from the subscription manager', async () => {
  let reads = 0
  const session = {accessToken:'test-token',accountId:'test-account'}
  const getAdapter = () => ({options:{tokens:{session:async () => {reads++;return session}}}})
  const provider = createCodexSearchProvider({
    resolveCredentials: async ({signal}) => {
      const value = await searchSession(getAdapter,signal)
      return {auth:{auth:{apiKey:value.accessToken}},credential:{type:'oauth',accountId:value.accountId}}
    },
    fetch: async (_,init) => {
  ...
  await provider.search({query:'test'},new AbortController().signal)
  assert.equal(reads,1)
})

test('aborting search releases its waiter while shared refresh remains usable', async () => {
  let resolve
  const pending=new Promise(r=>{resolve=r})
  const getAdapter=()=>({options:{tokens:{session:()=>pending}}})
  const controller=new AbortController()
  const first=searchSession(getAdapter,controller.signal)
  const second=searchSession(getAdapter)
  controller.abort()
  await assert.rejects(first,e=>e.name==='AbortError')
  const session={accessToken:'test-token',accountId:'test-account'}
  resolve(session)
  assert.equal(await second,session)
  await assert.rejects(searchSession(()=>({})),/manager is unavailable/)
})
