# DSH Mobile Session Sync

This out-of-tree Host plugin implements `mobile-session-sync-v2` and the additive
`mobile-session-sync-v3` surface for native clients:
...
All routes are loopback-only and do not change `dsh-core`. Initial history, scan-limit recovery, and
backward pagination continue to use `session.history`; the v2 routes own reconnect baselines and
strict incremental recovery only.

## `mobile-session-sync-v3` wire contract

The v3 surface is deliberately an out-of-tree, read-only transport. It does not add an
`ApiProxy` method and does not use the core RPC method registry.
...
summary, failure category, and call id; arguments/results that do not fit inline are addressable
through `detailRef: {seq, version, field}`. Details use an event sequence and version, not a
...
reloaded from bounded `session.history` pages when requested. A missing/reload failure is an
